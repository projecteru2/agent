...
// SetNode provides a mock function with given fields: ctx, node, status
func (_m *Store) SetNode(ctx context.Context, node string, status bool) error {
	ret := _m.Called(ctx, node, status)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string, bool) error); ok {
		r0 = rf(ctx, node, status)
	} else {
		r0 = ret.Error(0)
	}
